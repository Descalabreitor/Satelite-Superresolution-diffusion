...
class GaussianDiffusion(nn.Module):
    def __init__(self, denoise_fn, rrdb_net, aux_perceptual_loss, aux_l1_loss=False, timesteps=1000, loss_type='l1',
        ...
        if beta_schedule == 'cosine':
            betas = cosine_beta_schedule(timesteps, s=beta_start)
    ...
    @torch.no_grad()
    def interpolate(self, x1, x2, img_lr, img_lr_up, use_rrdb, t=None, lam=0.5):
    ...
    def res2img(self, img_, img_lr_up):
        img_ = img_.clamp(-1, 1)
        img_ = img_ / 2 + img_lr_up
        return img_
