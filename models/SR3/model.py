...
class UNet(nn.Module):
    ...
    def forward(self, x, gamma):
        ...
        for module in self.up:
            z = module(torch.cat((z, connections.pop()), dim=1), emb) if isinstance(module,
                                                                                    WideResNetBlock) else module(z)
...
class GammaEmbedding(nn.Module):
    ...
    def forward(self, gamma):
        ...
        emb = torch.concat((torch.sin(x), torch.cos(x)),
                           dim=1)  # sin( gamma / 5000^(i / (d/2 - 1)) ) and cos( gamma / 10000^(i / (d/2 - 1)) )
        if self.dim % 2 != 0:
...
class GroupNorm(nn.Module):
    def __init__(self, in_channel, num_groups=3):
        super().__init__()
...
class Conv2d(nn.Module):
    def __init__(self, in_channel, out_channel, kernel, stride=1, gain=1.0):
        ...
        nn.init.xavier_uniform_(self.conv.weight,
                                gain=torch.sqrt(torch.tensor(gain)))  # the original code initialization
