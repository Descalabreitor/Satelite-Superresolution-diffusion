...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, x_c):
        # Algorithm 2
        x_T = torch.randn(x_c.shape, device=x_c.device)
        x_c = self._norm(x_c)
        B, *_ = x_T.shape
        dimension_num = len(x_T.shape)

        x_t = x_T
        for step in reversed(range(self.sample_steps)):
            ...
            beta_over_sqrt_one_minus_alpha_bar = self._extract(self.sample_betas_over_sqrt_one_minus_alphas_bar, time,
                                                               dimension_num)
