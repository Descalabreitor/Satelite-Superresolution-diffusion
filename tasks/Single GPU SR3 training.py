...
def setUpTrainingObjects(config):
    ...
    model_builder = model_builder.set_beta_schedule(config["beta_schedule"])
    if config["beta_schedule"] == "cosine":
        model_builder = model_builder.set_steps(100)
...
def execute_check(config, test_dataloader, epoch, trainer, log_data_wandb, log_data_local):
    ...
    for id, image in enumerate(sr_images):
        image.save(
            f"C:\\Users\\adrianperera\\Desktop\\SR-model-benchmarking\\test pictures\\{config['model_name']}\\Epoch_{epoch}_{id}.png")
...
def execute(config):
    ...
    train_dataloader, val_dataloader, test_dataloader = setUpDataloaders(config, "C:\\Users\\adrianperera\\Desktop\\dataset_tfg")
    ...
    for epoch in range(config["start_epoch"] + 1, config['num_epochs']):
        ...
        utils.logger_utils.dict_to_csv(log_data_local,
                                       "C:\\Users\\adrianperera\\Desktop\\SR-model-benchmarking\\logs\\SR3" +
                                       config["model_name"])
...
if __name__ == "__main__":
    config = {
        ...
        "lr_size": 64,
        "hr_size": 256,
        "save_dir": "C:\\Users\\adrianperera\\Desktop\\SR-model-benchmarking\\saved models\\SR3\\version 6",
